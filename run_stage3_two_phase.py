"""
Chạy Stage 3: Full End-to-End Distillation (MOHAWK) — input từ Stage 2 TWO-PHASE.

Giống hệt run_stage3.py (cùng dùng distillation/stage3_finetune.py đã rebuild:
LR fix, BN recalibration, spike-revert có seed baseline, paper-aligned wd/clip),
chỉ khác checkpoint Stage 2 đầu vào:
    run_stage3.py          → checkpoints/student_stage2_best_nofreeze_v6.pth   (single-phase)
    run_stage3_two_phase.py → checkpoints/student_stage2_two_phase_best.pth     (two-phase, Phase B)

Mục đích: so sánh xem Stage 2 two-phase curriculum (freeze_mlp=True → False)
có cho Stage 3 khởi đầu tốt hơn single-phase hay không.

Output (checkpoint + wandb run) đặt tên riêng để chạy SONG SONG với
run_stage3.py đang chạy mà không bị đè file / log lẫn nhau.

Cách dùng (nếu máy có nhiều GPU, nên tách GPU để không OOM / không tranh tài
nguyên với run_stage3.py đang chạy):
    CUDA_VISIBLE_DEVICES=1 python run_stage3_two_phase.py
"""
...
# Khác run_stage3.py: dùng checkpoint Stage 2 TWO-PHASE (Phase B = full block,
# freeze_mlp=False, target=block_outputs) — xem run_stage2_two_phase.py
STUDENT_STAGE2_CKPT = "checkpoints/student_stage2_two_phase_best.pth"  # wandb: stage2-wlasl100-v7-two-phase (Phase B)
...
WANDB_NAME    = "stage3-wlasl100-v6-from-two-phase"  # v6: input từ Stage 2 two-phase (so sánh với v5-rebuild)
...
def main():
    ...
    if USE_WANDB:
        ...
        wandb_run = wandb.init(
            ...
            name=WANDB_NAME,
            config=dict(
                stage=3, stage2_variant="two_phase",
                ...
                patience=PATIENCE,
                stage2_ckpt=STUDENT_STAGE2_CKPT,
            ),
    ...
    # ── Student — load từ Stage 2 TWO-PHASE checkpoint ────────────────
    student_ckpt = STUDENT_STAGE2_CKPT
    if not os.path.exists(student_ckpt):
        alt = student_ckpt.replace(".pth", "_final.pth")
        if os.path.exists(alt):
            print(f"[INFO] {os.path.basename(student_ckpt)} not found, dùng: {alt}")
            student_ckpt = alt
        else:
            print(f"[ERROR] Stage 2 two-phase checkpoint không tồn tại: {student_ckpt}")
            print("Hãy chạy run_stage2_two_phase.py trước (Phase B → student_stage2_two_phase_best.pth).")
            sys.exit(1)

    print(f"\nLoading student từ Stage 2 (two-phase): {student_ckpt}")
    student = BiMambaSLR(
        in_channels=IN_CHANNELS,
        num_classes=num_classes,
        seq_len=SEQ_LEN,
    ...
    # Sanitize params và buffers từ Stage 2 (giống run_stage3.py)
    ...
    # Output đặt tên riêng để KHÔNG đè lên checkpoints/student_stage3.pth
    # của run_stage3.py đang chạy song song.
    save_path = os.path.join(OUTPUT_DIR, "student_stage3_two_phase.pth")
    ...
    print(f"\n✓ Stage 3 (two-phase input) xong → {save_path}")
